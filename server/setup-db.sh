...
# Setup MySQL Database for AlphaAIM

echo "=== AlphaAIM MySQL Database Setup ==="
echo ""
...
if [ $? -eq 0 ]; then
    ...
    echo "  Host: localhost"
    echo "  Database: alphaaim_db"
    echo "  User: root (MySQL root user)"
else
    echo ""
    echo "❌ Database setup failed. Please check MySQL is running and accessible."
fi
